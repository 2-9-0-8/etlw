...
export async function getEpisodePositionInSeries(id: string, series: number) {
	const episodes: Episode[] = await sanityClient.fetch(
		`*[_type == 'episode' && series == $series] {
      _id,
      _createdAt
    }`,
		{ series },
	...
	return sorted.findIndex(e => e._id === id) + 1
}
